...
| --- | --- | --- | --- |
| EVT-001 | **Resolved** | Versioned domain envelope, transactional PostgreSQL outbox/inbox, bounded dispatcher retry/DLQ, deterministic jobs, dual new-message/reclaim Redis subscribers, and operator replay commands. All durable tables ship in the single `0003_backend_reliability` revision. | `app/core/tests/unit/test_reliability_foundations.py`; `app/core/infrastructure/events/tests/unit/test_stream_subscriber.py`; schedule E2E: 16 passed; datastore/apps E2E union: 90 passed, one protected GCS skip. |
...
| USAGE-001 | **Resolved** | Canonically ordered multi-scope counter upsert/locking, one-transaction admission/reservation/record/release, deduplication, and PostgreSQL `NULLS NOT DISTINCT` uniqueness. Existing duplicates are merged by revision `0003`. | `test_usage_concurrency_e2e.py` (2); `test_usage_limits_fake_unit.py`; `test_system_pricing_unit.py`; `test_usage_entities.py`. |
| SCHEDULE-001 | **Resolved** | Durable fire ledger keyed by schedule/source event, stable source IDs, failed/retryable filter outcomes, outbox target publication, retry/list APIs, and durable consecutive-failure state. Redis fire-store code was removed. Tables/column are in revision `0003`. | Full schedule E2E: 16 passed; `test_schedule_processor.py`; `test_schedule_idempotency_regression.py`; aggregate schedule coverage 77.7% (1,570/2,021). |
...
| TEST-001 | **Resolved** | CI aggregates unit plus five E2E coverage shards and enforces 70% overall, 65% schedule, 90% changed-code, plus per-module ratchet reporting. | Local aggregate: 74.32% overall, 77.7% schedule, exactly 90% changed code; full unit/component suite: 2,101 passed, one optional skip. |
...
| OBS-001 | **Partial** | Event/correlation/request identifiers, outbox/inbox attempts/outcomes, retry/DLQ metrics, worker watchdog, schedule fire state, bundle checkpoints, and operator replay/SLO guidance are implemented. A production dashboard/alert deployment is environment-owned and not part of this repository diff. | `docs/operators/reliability.md`; reliability unit tests; schedule fire API/E2E; outbox dispatcher tests. |
...
Persist a schedule-fire ledger keyed by `(schedule_id, source_event_id)` with
...
| --- | --- |
| `make test-unit` | 2,101 passed, one optional MarkItDown test skipped, 503 E2E deselected |
| Aggregate unit + targeted E2E coverage | 74.32% total; schedule 77.7%; changed code 90%; all configured floors passed |
...
| `pytest app/modules/schedule/tests/e2e` | 16 passed, including cron, webhook, datastore, fire ledger, retry, authorization, and telemetry |
