...
async def test_fire_ledger_concurrent_dedup_and_retry_api(
    ...
    async def claim_once():
        async with factory() as uow:
            return await ScheduleFireRepository(uow).claim(
    ...
    async with factory() as uow:
        await ScheduleFireRepository(uow).mark_failed(
            fire.id, RuntimeError("provider unavailable")
        )

    listed = await authenticated_client.get(
        f"/pods/{pod_id}/schedules/{schedule_id}/fires"
    )
    ...
    retried = await authenticated_client.post(
        f"/pods/{pod_id}/schedules/{schedule_id}/fires/{fire.id}/retry"
    )
