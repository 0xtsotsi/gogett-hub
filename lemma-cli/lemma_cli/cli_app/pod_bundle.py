...
# schedule/surface account_id. The templating/resolution lives in the shared
# lemma-pod-bundle package; the thin wrappers below keep the CLI call sites
# unchanged while the backend reuses the same implementation.
...
def _extract_portable_variables(bundle_root: Path) -> dict[str, Any]:
    ...
    placeholders and record them under ``pod.json -> variables``. Delegates to
    the shared lemma-pod-bundle implementation — one manifest story across the
    CLI and backend."""
    from lemma_pod_bundle import extract_portable_variables

    return extract_portable_variables(bundle_root)


def _strip_unresolved_placeholders(node: object) -> object:
    """Drop dict entries whose value is still an unresolved ``${...}`` token so a
    literal placeholder never reaches the API (e.g. an unsupplied account)."""
    from lemma_pod_bundle.portability import strip_unresolved_placeholders

    return strip_unresolved_placeholders(node)
