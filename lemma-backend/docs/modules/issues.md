...
| SCHEDULE-001 | **Resolved** | Durable fire ledger keyed by schedule/source event, stable source IDs, failed/retryable filter outcomes, outbox target publication, retry/list APIs, and durable consecutive-failure state. Redis fire-store code was removed. Tables/column are in revision `0003`. | Full schedule E2E: 16 passed; `test_schedule_processor.py`; `test_schedule_idempotency_regression.py`; `test_reliability_adapters.py`; aggregate schedule coverage 75.41% (1,524/2,021). |
